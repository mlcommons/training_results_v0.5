  ...
  def _get_decay_info(self, hparams):
    ...
    if hparams.decay_scheme in [
      ...
      if hparams.decay_scheme == "luong5":
        ...
        epoch_size = iterator_utils.get_effective_train_epoch_size(hparams)
        decay_times = 16
        start_decay_step = int(hparams.max_train_epochs * epoch_size / 16. /
                               hparams.batch_size)
        remain_steps = int(hparams.max_train_epochs * epoch_size /
                           hparams.batch_size) - start_decay_step
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        if not hparams.use_fused_lstm_dec:
          ...
          final_rnn_outputs = self._build_decoder_fused_for_training(
              encoder_outputs, encoder_state, decoder_emb_inp, self.hparams)
      ...
      ## Inference
        ...
        # It's embedded in tf library.
        mlperf_log.gnmt_print(key=mlperf_log.EVAL_HP_LEN_NORM_CONST, value=5)
        ...
        mlperf_log.gnmt_print(key=mlperf_log.EVAL_HP_BEAM_SIZE,
                              value=beam_width)
        # Use static max_iter for now.
        # TODO(jamesqin): fix it.
        mlperf_log.gnmt_print(key=mlperf_log.EVAL_HP_MAX_SEQ_LEN,
                              value=hparams.tgt_max_len_infer)
