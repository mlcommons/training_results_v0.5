...
std::vector<at::Tensor> nhwc_bn_fwd_train_cudnn_impl(
  ...
  at::Tensor y = at::empty_like(x); // ({N, H, W, C}, x.options());
  ...
  at::Tensor reserve = at::empty({static_cast<long>(reserve_num_bytes)}, torch::CUDA(at::kByte));

  // call cuDNN
  float one = 1.f, zero = 0.f;
  auto eps = std::max(epsilon, BN_MIN_EPSILON);
  at::Tensor save_mean = at::empty({C}, scale.options());
  at::Tensor save_var = at::empty({C}, scale.options());
...
std::vector<at::Tensor> nhwc_bn_bwd_cudnn_impl(
  ...
  if (fuse_add) {
    ...
    z_grad = at::empty({}, torch::CUDA(at::kFloat));
  }
