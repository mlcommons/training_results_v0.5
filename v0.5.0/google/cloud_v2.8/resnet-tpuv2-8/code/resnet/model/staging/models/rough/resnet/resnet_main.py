...
from resnet import async_checkpoint
from resnet import imagenet_input
from resnet import in_memory_eval
from resnet import resnet_model
from resnet import eval_runner
...
flags.DEFINE_float(
    'stop_threshold', default=0.749, help=('Stop threshold for MLPerf.'))
...
def resnet_model_fn(features, labels, mode, params):
  ...
  mlperf_log.resnet_print(key=mlperf_log.INPUT_MEAN_SUBTRACTION, value=MEAN_RGB)
  ...
  mlperf_log.resnet_print(key=mlperf_log.MODEL_EXCLUDE_BN_FROM_L2, value=True)
  mlperf_log.resnet_print(
      key=mlperf_log.MODEL_L2_REGULARIZATION, value=FLAGS.weight_decay)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    mlperf_log.resnet_print(key=mlperf_log.OPT_MOMENTUM, value=FLAGS.momentum)
    if FLAGS.use_tpu:
...
def main(unused_argv):
  ...
  mlperf_log.resnet_print(
      key=mlperf_log.RUN_SET_RANDOM_SEED, value="none")
  ...
  mlperf_log.resnet_print(
      key=mlperf_log.INPUT_BATCH_SIZE, value=FLAGS.train_batch_size)
  ...
  mlperf_log.resnet_print(
      key=mlperf_log.PREPROC_NUM_TRAIN_EXAMPLES, value=FLAGS.num_train_images)
  mlperf_log.resnet_print(
      key=mlperf_log.PREPROC_NUM_EVAL_EXAMPLES, value=FLAGS.num_eval_images)
  ...
  if FLAGS.mode == 'eval':
    ...
    if FLAGS.use_eval_runner:
      runner = eval_runner.EvalRunner(input_fn=imagenet_eval.input_fn,
                                      model_fn=resnet_model_fn,
                                      params=params,
    ...
    for ckpt in evaluation.checkpoints_iterator(
      ...
      try:
        ...
        if FLAGS.use_eval_runner:
          eval_results = runner.eval(num_steps=eval_steps, checkpoint_path=ckpt)
        else:
        ...
        mlperf_log.resnet_print(
            key=mlperf_log.EVAL_SIZE, value=FLAGS.num_eval_images)
    ...
    if FLAGS.mode == 'train':
      ...
      while current_step < FLAGS.train_steps:
        ...
        next_checkpoint = min(current_step + FLAGS.steps_per_eval,
                              FLAGS.train_steps)
        mlperf_log.resnet_print(
            key=mlperf_log.TRAIN_EPOCH, value=current_step // steps_per_epoch)
        resnet_classifier.train(
