...
def get_effective_train_epoch_size(hparams):
  ...
  for srcline, tgtline in zip(srcf, tgtf):
    if (
        len(srcline.split()) < hparams.src_max_len and
        len(tgtline.split()) < hparams.tgt_max_len):
      epoch_size += 1
  srcf.close()
  tgtf.close()
  return epoch_size
...
def get_iterator(src_dataset,
  ...
  use_xla_compile = os.environ["use_xla_compile"] == "true"
  ...
  def batching_func(x):
    return x.padded_batch(
        ...
        padding_values=(
            ...
            0,  # src_len -- unused
            0),
        drop_remainder=use_static_input_shape)
